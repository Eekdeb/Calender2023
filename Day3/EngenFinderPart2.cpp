...
int addNumbers(string* upperrow, string* midlerow, string* lowerRow){
    ...
    int resultingGears = 0;
    list<int> foundNumbers{};
    for (auto i : symbolIndex) {
        //check Upper row
        valuesNextToSymbols = takeNumber(upperrow,i-1);
        if(valuesNextToSymbols != 0)
            foundNumbers.push_back(valuesNextToSymbols);
        ...
        if(foundNumbers.size() == 2){
            cout <<  foundNumbers.front() << "   " << foundNumbers.back() << "\n";
            resultingGears += foundNumbers.front() * foundNumbers.back();
        }
        foundNumbers.clear();
    }
    return resultingGears;
}
